...
class TaskJob_Finance_Bills_Family(TaskJob_Finance):
    def update(self):
        todoist = self.get_todoist()
        proj = self.get_project()
        sect = self.get_section_by_name(proj.id, "Bills")

        # set up a TaskConfig object for the task
        content_fname = __file__.replace(".py", ".md")
        t = TaskConfig()
        t.parse_json({
            "title": "Pay the Family Bills",
        ...
        due = dtu.get_last_day_of_month(now)
        last_friday_diff = dtu.get_days_since_weekday(due, dtu.Weekday.FRIDAY)
        due = dtu.add_days(due, -1 * last_friday_diff)
