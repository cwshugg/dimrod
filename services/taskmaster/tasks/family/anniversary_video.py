...
class TaskJob_Family_AnniversaryVideo(TaskJob_Family):
    def update(self, todoist, gcal):
        ...
        t = TaskConfig()
        t.parse_json({
            "title": "Record Yearly Anniversary Video",
        ...
        if last_success is not None and dtu.diff_in_weeks(now, last_success) < 30:
            return False
        if now.month != 5:
            return False
        if now.day not in range(15, 31):
            return False

        # retrieve the task (if it exists) and select an appropriate due date
        task = todoist.get_task_by_title(t.title, project_id=proj.id)
        due = now.replace(day=20)
