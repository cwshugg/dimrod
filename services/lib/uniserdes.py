# Module that defines a "Universal Serializable/Deserializable" object that is
# used across the codebase.
#
# This is used to represent all sorts of data in various forms (JSON, SQLite3,
...
class Uniserdes:
    ...
    # Statically initializes a new Uniserdes object, given a file path.
    @classmethod
    ...
    # Statically initializes a new Uniserdes object, given a JSON dictionary.
    @classmethod
    ...
    def parse_json(self, jdata: dict, base_path: str = None):
        ...
        for f in self.fields:
            ...
            if key in jdata:
                # if the expected type is actually a sub-class of this Uniserdes
                # class, then we have a nested uniserdes object
                val = None
                if issubclass(types[0], Uniserdes):
                    cls = types[0]
                ...
                if val is None and issubclass(types[0], Enum):
                    ...
                    # or, attempt to convert from a string
                        ...
                        # be forgiving of surrounding whitespace and case when
                        # looking up the enum by name
                        val = cls[enum_val.strip().upper()]
                    else:
    ...
    def get_sqlite3_table_definition(self, table_name: str,
        ...
        for (i, name) in enumerate(fields_to_keep_visible):
            ...
            # make sure this field has a corresponding `UniserdesField` object
            field = self.get_field(name)
            assert field is not None, \
                "the field \"%s\" does not have a defined UniserdesField object" % name
